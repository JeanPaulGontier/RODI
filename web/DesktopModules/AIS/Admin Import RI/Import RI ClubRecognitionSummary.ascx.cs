    ...
    void Import_Fichier()
    {
        ...
        try
        {
            ...
            foreach(var cr in crs)
            {
                ...
                {
                    Yemon.dnn.DataMapping.ExecSqlNonQuery(new SqlCommand("DELETE FROM ["+Const.TABLE_PREFIX+"ri_clubrecognitionsummary] WHERE districtid="+cr.districtid+" AND cric="+cr.cric+" AND annee="+cr.annee),conn,trans);
                    ...
                    Yemon.dnn.DataMapping.UpdateOrInsertRecord(Const.TABLE_PREFIX+"ri_clubrecognitionsummary", "id", r, conn, trans);
